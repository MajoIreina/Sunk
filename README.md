...
Windows `.lnk` shortcuts and directly dropped `.exe` files enter the application-identification path rather than being deleted as ordinary files. Sunk resolves a classic shortcut, ranks eligible current-user and local-machine uninstall records from both registry views, and deterministically selects the strongest match. A Chinese confirmation dialog shows the matched application, unverified registry publisher, installation location, registered uninstall source, and dropped source. Once the user confirms, that captured Win32 or MSI launch plan runs directly when the visual reaches the operation boundary; it is not discarded by a second registry scan. Commands are parsed into an executable and arguments and are launched without `cmd.exe` or another shell. UWP/MSIX/AppRef and `.url`/`.website` links, missing records, invalid MSI identities, script or shell hosts, network/relative launch paths, and missing local uninstallers remain unsupported. Sunk never deletes an application installation directory itself.

The interaction distinguishes hover, radial attraction/capture, infall with near-horizon slowdown, apparent-horizon waiting, and real success or failure. The path is a zero-angular-momentum Schwarzschild-style radial infall: the object does not orbit, spiral, or rotate as a rigid sprite. Increasing tidal gradient stretches it radially and compresses it tangentially. An application visual pauses until the user confirms. A file operation begins only when its visual reaches the rendered shadow boundary, then holds there with gravitational redshift until the worker returns a result. Success lets an external visual cross that boundary; its `4x4` image tiles are progressively occluded by the rendered shadow so the object disappears piece by piece after entering the horizon instead of shrinking to a point. A visual released while already overlapping the shadow starts at its real release point and moves only inward. Cancellation or failure produces a visible rejection instead of a false consume animation.

While an Explorer object is held inside the lens influence range, Sunk applies a short radial tidal echo to the same captured desktop pixels and geodesic support used by ordinary background lensing. This is visual feedback only: releasing in the wider influence field does not authorize an operation; the pointer must still be inside the shadow or emitting-disk drop target.
...
| **画质** | 30/60/120 FPS limit, geodesic integrator quality, and an independent anti-aliasing selector |
...
An external OLE file drag has its own ownership state because its mouse press originates in Explorer, not in the Bevy window. Global button state lets the overlay acquire an incoming drag in the lens influence field before the first OLE hover event, without expanding ordinary click ownership. While the drag remains active, native resizing is deferred and the physical feedback follows the native cursor. Moving outside the actual drop target removes its acceptance response, and the release position is checked again before a batch is created. File events addressed to the settings window are ignored by the primary drop coordinator.
...
so density is not accidentally applied twice. The radial temperature follows a zero-torque thin-disk profile. Inside the ISCO, a tenuous plunging atmosphere and continuous blend from circular Doppler shift to radial-infall redshift avoid an artificial hard gap at the shadow. A Planckian-locus approximation supplies black-body chromaticity; Schwarzschild gravitational shift and orbital or plunging motion alter observed temperature and brightness. The user tint is applied afterward in linear RGB.
...
The Windows proof backend captures a physical client-area region with up to 45% overscan on every side and uploads an sRGB desktop texture at 30 FPS. Each render frame remaps that absolute captured rectangle to the overlay's current desktop coordinates, so moving the window does not wait for the next GDI frame to realign the lens. GDI capture is constrained to the monitor containing the renderer because a single `BitBlt` rectangle spanning multiple displays or adapters is not reliable; uncovered areas remain transparent. If the expanded copy fails, it temporarily falls back to the client region and periodically retries overscan.
...
The rollback branch `backup/pre-file-operations-20260816` points to `8864a41`, the complete native renderer immediately before file operations. `backup/pre-desktop-polish-20260816` and `backup/pre-tidal-infall-20260816` both preserve `ed30783`, immediately before the current desktop, tray, frame-rate, uninstall, and tidal-interaction pass.
